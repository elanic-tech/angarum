...
module.exports = Template.extend('FedEx', {
    ...
    order: function(params, cb) {
		var inp = params.get();
		var date = new Date();
        var from_mobile_number = inp.from_mobile_number;
        var to_mobile_number = inp.to_mobile_number;
		...
		var data = {
		  RequestedShipment: {
		    ...
		    TotalWeight : [{
		    	Units : 'KG',
		    	Value : (inp.delivery_type && inp.delivery_type === 'surface') ? '5' : '.400'
		    }],
		    Shipper: {
		      Contact: {
		        PersonName: inp.from_name,
		        PhoneNumber: from_mobile_number
		      },
		      Address: {
		        StreetLines: [
		        	from_street_line_1,
